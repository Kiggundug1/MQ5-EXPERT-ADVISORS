...
int OnInit()
{
   ...
   if(handle != INVALID_HANDLE)
   {
      ...
      {
         // if shadow copies are assigned, decode them and override periods
...
int OnTesterInit()
{
   ...
   && ParameterGetRange("SlowOsMA", enabled2, value2, start2, step2, stop2))
   {
      if(enabled1 && enabled2)
      {
         ...
         // now check if the shadow file already exists - otherwise it won't be sent to agents
         const bool preExisted = FileIsExist(SettingsFile);
         
         // write required settings to the file for sending to our copy running on agents
         ...
         if(!preExisted)
         {
            PrintFormat("Required file %s is missing. It has been just created. Please restart again.",
               SettingsFile);
            ChartClose();
            return INIT_FAILED;
         }
